...
/** Single-plot NDVI trend popup — the HTML's "Individual plots" chart mode
 * (RS_Cane_Monitoring_S1.html:5400-5495), scoped to one plot on demand
 * rather than all plots at once on the main tab (see plan's Chart modes
 * decision). Opened by clicking a Field Card's sparkline or the Field Map
 * detail panel's "View full trend" button. */
export function NdviTrendModal({ field, geo, onClose }: NdviTrendModalProps) {
      ...
      <div className="w-full max-w-2xl rounded-lg bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-start justify-between">
          <div>
            ...
            <div className="text-xs text-neutral-400">{field.code} · NDVI trend — age in days after planting</div>
          </div>
        ...
        {points.length === 0 ? (
          <div className="p-10 text-center text-sm text-neutral-400">No observation history for this plot.</div>
        ) : (
          <>
            ...
            {stats.unconfirmed > 0 && (
              <div className="mb-3 text-[10px] text-neutral-400">
                {stats.unconfirmed} unconfirmed reading{stats.unconfirmed === 1 ? '' : 's'} (cloud/S1 gap-fill) shown
                in grey, excluded from the counts above.
              </div>
            )}
